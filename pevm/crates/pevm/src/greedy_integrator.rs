...
impl GreedyIntegrator {
    ...
    /// Main greedy integration algorithm
    /// Input: A set of dependency graphs
    /// Output: A list of integrated graphs
    pub fn greedy_decent_integration(
        ...
        // Sort graphs in descending order of CV
        graphs.sort_by(|a, b| {
            let cv_a = a.simulation_result.as_ref()
                .map(|r| r.coefficient_of_variation)
                .unwrap_or(0.0);
            let cv_b = b.simulation_result.as_ref()
                .map(|r| r.coefficient_of_variation)
                .unwrap_or(0.0);
            cv_b.partial_cmp(&cv_a).unwrap_or(std::cmp::Ordering::Equal)
        });
    
        let mut l = graphs; // L: list of graphs
        let mut output = Vec::new(); // O: output list
    
        while !l.is_empty() {
            // G ← L[0] (initialize with highest-CV graph)
            let mut g = l.remove(0);
            
            // S: set of graphs merged into G
            let mut s_indices = HashSet::new();
            ...
            while i > 0 {
                ...
                let (is_decent, integrated_graph) = self.decent_integration(&g, &l[i], &integrated_graphs);
                
                if is_decent {
                    // Use the already-computed integrated graph
                    g = integrated_graph;
                    s_indices.insert(i);
                    integrated_graphs.push(l[i].clone());
                    
                    println!(
                        "Integrated graph {} into G. New CV: {:.4}",
                        i,
            ...
            // Remove all graphs in S from L (in reverse order to maintain indices)
            let mut indices_to_remove: Vec<_> = s_indices.into_iter().collect();
