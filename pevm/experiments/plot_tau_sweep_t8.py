"""tau_cv/threshold sweep at NUM_THREADS=8 over 200 batches (real ETH).
...
SWEEP_DIR = Path("/home/ubuntu/congestion_control/pevm/experiments/tau_sweep_t8")
OUT = Path("/home/ubuntu/congestion_control/pevm/experiments/plot_tau_sweep_t8.png")
...
for v, color in zip(VALUES, colors):
    csv = SWEEP_DIR / f"batches_tau_{v}.csv"
    if not csv.exists(): print(f"  missing {csv}"); continue
    df = pd.read_csv(csv)
    sp = df["integrated_tput"] / df["seq_tput"]
    ax.plot(df["batch_idx"], sp, label=f"tau_cv = {v}", color=color, linewidth=1.3)
...
ax.set_title("Integrated speedup across tau_cv sweep  (real ETH, 200 batches, NUM_THREADS=8, hot_key_threshold=1.5)")
ax.grid(alpha=0.3); ax.legend(loc="upper right", fontsize=9)
allv = np.concatenate([(pd.read_csv(SWEEP_DIR/f"batches_tau_{v}.csv")["integrated_tput"]/
        pd.read_csv(SWEEP_DIR/f"batches_tau_{v}.csv")["seq_tput"]).dropna().values
        for v in VALUES if (SWEEP_DIR/f"batches_tau_{v}.csv").exists()])
