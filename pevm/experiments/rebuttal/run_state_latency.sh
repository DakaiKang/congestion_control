...
export START_BLOCK=19557289 BATCH_SIZE=100 MAX_BATCHES=${MAX_BATCHES:-20} NUM_THREADS=${NUM_THREADS:-8} TAU_CV=0.5 HOT_KEY_THRESHOLD=1.5
cargo build --release --test rebuttal_test >/dev/null 2>&1
for d in ${DELAYS:-0 1000 2000 5000 10000 20000}; do
  mkdir -p ${REB_ROOT:-experiments/rebuttal}/sweeps; f=${REB_ROOT:-experiments/rebuttal}/sweeps/statelat_real_d${d}${SUFFIX:-}.csv
