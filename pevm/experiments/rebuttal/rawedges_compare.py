...
none_diag = load([f"{D}/rawedges/none_diag_{c}.csv" for c in chunks], NB)  # baseline rerun with cross-block counters, if present
sets = {"none (baseline)": (load([f"{D}/real_{c}.csv" for c in chunks], NB),
                            none_diag if none_diag is not None else load([f"{D}/real_{c}_diag.csv" for c in chunks], NB))}
...
print(f"{'RAW edges':16s}{'engine':14s}{'rounds':>7}{'speedup':>9}{'re-exec/tx':>11}{'val/tx':>8}{'cascade/tx':>11}{'blocking/tx':>12}{'cross-blk val':>14}{'cross-blk casc':>15}")
for mode, (t, d) in sets.items():
    for kt, kd, name in eng:
        ...
        if d is not None and f"{kd}_cascade_aborts" in d:
            ...
            xb = f"{d[f'{kd}_cross_block_aborts'].sum() / n:>14.3f}{d[f'{kd}_cross_block_cascade'].sum() / n:>15.3f}" if f"{kd}_cross_block_aborts" in d else f"{'-':>14}{'-':>15}"
            print(f"{mode:16s}{name:14s}{len(d):>7}{sp:>9}{r:>11.3f}{v:>8.3f}{c:>11.3f}{r - v:>12.3f}{xb}")
        else:
