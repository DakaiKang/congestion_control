...
NB=${NB:-20}
run() { # $1 mode (suffix B = TRACK_BASIC_READS=1) $2 build
  local mode=$1 build=$2 feat="" tb=0 gm=$1; [ $build = diag ] && feat="--features diagnostics"
  case $mode in *B) tb=1; gm=${mode%B};; esac
  for start in 16774645 18581726 19557289; do
    f="$OUT/${mode}_${build}_${start}.csv"
    [ -s "$f" ] && [ "$(wc -l < "$f")" -gt "$NB" ] && { echo "skip $f"; continue; }
    echo "==> $f  $(date +%H:%M)"
    TRACK_BASIC_READS=$tb GRAPH_EDGES=$gm START_BLOCK=$start MAX_BATCHES=$NB OUTPUT="$f" cargo test --release $feat --test rebuttal_test \
